...
class LRFinder(LRUpdater):
    ...
    def on_train_begin(self, state: DotDict) -> None:
        ...
        torch.save(state.core.model.state_dict(), 'tmp/tmp.pth')
        self.update_lr(self.init_lr, state.core.opt)
        
    def on_batch_end(self, i: int, state: DotDict) -> None:
        super().on_batch_end(i, state)
        self.smoother.add_value(to_numpy(state.core.loss))
        ...
        self.loss_dict['loss'].append(to_numpy(state.core.loss))
    ...
    def on_train_end(self, state: DotDict) -> None:
        ...
        if not isinstance(state.core.model, DataParallelModel) \
                and "module." in list(checkpoint.keys())[0]:
            # [7:] is to skip 'module.' in group name
            checkpoint = {k[7:]: v for k, v in checkpoint.items()}

        load_state_dict(model=state.core.model,
                        state_dict=checkpoint,
                        skip_wrong_shape=False)
        if hasattr(state.core.model, 'reset'): state.core.model.reset()
        shutil.rmtree('tmp/')
        del checkpoint
        gc.collect()
        self.plot()