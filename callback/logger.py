        ...
        if state.mode == "train":
            ...
            if self.is_tensorboard_available:
                ...
                self.train_writer.add_scalar("batch/loss_train",
                                             float(to_numpy(state.loss)),
                                             global_step=self.train_iter)
                self.train_epoch['loss'].append(float(to_numpy(state.loss)))
                
            else:
                self.train_writer.update(','.join([str(to_numpy(state.loss)), str(lr)]), i+1)
            ...
            if self.is_tensorboard_available:
                ...
                self.val_epoch['loss'].append(float(to_numpy(state.loss)))
            
            else:
                self.val_writer.update(str(to_numpy(state.loss)), i+1)
                self.metric_writer.update(','.join([str(x) for x in list(state.metrics['val'].values())]), i+1)
